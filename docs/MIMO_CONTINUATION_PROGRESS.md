...
## Codex Astra batch — E2.1 bounded QA direction/failure fix (2026-09-12)

Preserved controller-committed E1.2 `0ddb929` and unrelated evidence. Read latest ROOT_DIAGNOSIS and Sol E2 report/test; no applicable AGENTS.md. Added shared exported best-dot eight-direction selector, finite/epsilon fail-closed behavior and movement-gated sprint. Factory and scoped fight orchestrator now stop navigation failure with actual snapshot/target/status through existing FightStopError catch; caller wiring and real factory continuation tests cover no later targets/attacks. No game or route changes.

Corrected baseline 2 pass / 4 fail against 0ddb929 helpers; final focused navigation/monitor/LOS/dispatch plus real fixed-dt Sim/input queue tests 44/44, typecheck PASS. Initial mistaken expectation that recorded yaw/target should select S was corrected: required algorithm still selects S+D there. Initial and corrected logs retained under docs/e21. Boundary tests expose the real quantization defect; no obstacle-planning fix or browser proof claimed. Four known boss-fight-tick failures untouched. No browser/build/full route/install/agents/global changes/git mutation. Full deliverable and scope: E21_FINAL.md. Patch left for controller review; stop here pending separately authorized probe.
